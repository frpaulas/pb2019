#!/usr/bin/env node
/**
 * Build opening_sentences.json from DPB files in src/lib/data/opening_sentences/dpb/
 * Usage: node scripts/build-opening-sentences-json.cjs
 */

const fs = require('fs');
const path = require('path');

const dpbDir = path.join(__dirname, '..', 'src', 'lib', 'data', 'opening_sentences', 'dpb');
...
/**
 * Parse a single opening sentence DPB file
 */
function parseOpeningSentenceDpb(content) {
	const blocks = [];
	...
	while (i < lines.length) {
		...
		if (baseDirective === 'st') {
		...
		} else if (baseDirective === 'scripture') {
			// scripture:reference:: text
			const afterDirective = trimmedLine.substring(trimmedLine.indexOf(':') + 1);
			const doubleColonIndex = afterDirective.indexOf('::');

			let reference = '';
			let text = '';

			if (doubleColonIndex > -1) {
				reference = afterDirective.substring(0, doubleColonIndex).trim();
				text = afterDirective.substring(doubleColonIndex + 2).trim();
			} else {
				reference = afterDirective.trim();
			}
			...
			if (text) {
				block.text = parseTextWithWrappers(text);
			}
...
// Main build
console.log('📖 Building opening_sentences.json from DPB files...\n');
...
for (const file of dpbFiles) {
	...
	try {
		...
		const blocks = parseOpeningSentenceDpb(content);

		openingSentencesJson[name] = blocks;
		converted++;
...
// Write opening_sentences.json
fs.writeFileSync(outputJsonPath, JSON.stringify(openingSentencesJson, null, '\t'));

console.log(`\n\u2705 Built opening_sentences.json with ${converted} entries`);
if (errors > 0) {
