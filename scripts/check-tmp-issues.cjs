...
function main() {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.startsWith('===')) {
			...
			// Skip Hebrew section markers (Title case, single word)
			if (trimmed && /^[A-Z][a-z]+$/.test(trimmed) && trimmed.length > 1) {
				continue;
			}
			// Skip Latin section titles (contains lowercase, no asterisk, not just a number)
			if (
				trimmed &&
				!/^\d+$/.test(trimmed) &&
				!/\*/.test(trimmed) &&
				/[a-z]/.test(trimmed) &&
				lineBuffer.length % 3 === 0
			) {
				continue;
			}
			if (trimmed) {
	...
	for (let i = 0; i < showCount; i++) {
		...
		console.log(
			`Total verse lines: ${issue.total} (${issue.remainder} extra line${issue.remainder > 1 ? 's' : ''})`
		);
	...
	issues.forEach((issue) => {
		console.log(
			`  Psalm ${issue.psalm.padEnd(4)} - Line ${issue.startLine.toString().padStart(5)} - ${issue.remainder} extra line(s)`
		);
	});
}
